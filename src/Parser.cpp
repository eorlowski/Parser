...
long Parser::parse(string s, string& errorMessage, int recursionLevel, int* stringPos) {
	...
	for (i = 0; i < s.length(); i++) {
		...
		if (s[i] == '(') {
			...
			long subResult = parse(s.substr(i+1), errorMessage, ++recursionLevel, &relStringPos);
			if (errorMessage != "") {
				return i;
			}
...
//				cout << "Syntax error: ')' without corresponding '('" << endl;
				errorMessage = "Syntax error: ')' without corresponding '('";
				return i;
//				exit(1);
			}
			*stringPos = i;
